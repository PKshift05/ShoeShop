...
const ValueFilter = ({
  ...
  const handleRangeSearch = (e) => {
    ...
    if (errorRange) toast.error('Vui lòng điền khoảng giá phù hợp', {
      ...
      const ProductFilter = productsCategoryRedux.items.filter((item) => (item.price >= Number(priceRange.from || minValueFilter) && item.price <= Number(priceRange.to || maxValueFilter)))
      setCurrentPage(1)
